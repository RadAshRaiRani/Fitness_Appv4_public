...
export default function WorkoutPage() {
  ...
  const loadWorkoutPlan = useCallback(async () => {
    ...
    if (storedPlan) {
      console.log("✅ Found workout plan in localStorage");
      setPlan(storedPlan);
      setStatus("Workout Plan");
      setIsGenerating(false);
      return;
    }
    ...
    if (user?.id) {
      try {
        console.log("🔍 Checking database for workout plan...");
        setStatus("Loading workout plan...");
        ...
        if (response.ok) {
          ...
          if (data.plan?.workout_plan) {
            console.log("✅ Found workout plan in database");
            const workoutPlan = data.plan.workout_plan;
            setPlan(workoutPlan);
            setStatus("Workout Plan");
            setIsGenerating(false);
    ...
    console.log("🚀 No plan found - generating new workout plan with existing goals");
    generateWorkoutPlan();
  }, [user?.id]);
  ...
  useEffect(() => {
    ...
    // Load workout plan: check localStorage first, then database
    loadWorkoutPlan();
  }, [loadWorkoutPlan]);
  ...
  const handleGoalsSubmit = (goals: string) => {
    ...
    generateWorkoutPlan(goals);
  };

  const generateWorkoutPlan = async (customGoals?: string) => {
    try {
      ...
      const goals = customGoals || localStorage.getItem("userGoals") || "balanced training and strength";
          ...
          if (line.startsWith("data: ")) {
            try {
              ...
              if (data.event === "status") {
                ...
                workoutContent = data.content;
                setPlan(data.content);
                setStatus("Workout Plan Generated");
                setIsGenerating(false);
                
                // Save to localStorage
                localStorage.setItem("workoutPlan", data.content);
              } else if (data.event === "diet_complete") {
                // Ignore diet events on workout page
                console.log("🍽️ Diet plan received (ignoring)");
      ...
      setStatus("Failed to generate workout plan");
      setIsGenerating(false);
    }
  };
      ...
      <div className="min-h-screen bg-gradient-to-br from-purple-900 via-gray-900 to-pink-900 text-white">
        ...
        {!plan && !isGenerating && (
          <div className="text-center py-20">
            <p className="text-xl text-gray-400">No workout plan available</p>
            <button
              onClick={() => {
                const storedGoals = localStorage.getItem("userGoals");
                if (!storedGoals) {
